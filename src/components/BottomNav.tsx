...
export const BottomNav = () => {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-gray-200 py-2 px-6 flex justify-between items-center z-40 pb-safe">
      <Link 
        to="/"
        className={`flex flex-col items-center gap-1 min-w-[60px] p-1 ${isActive('/') ? 'text-red-600' : 'text-gray-400'}`}
      >
      ...
      <Link 
        to="/catalogo"
        className={`flex flex-col items-center gap-1 min-w-[60px] p-1 ${isActive('/catalogo') ? 'text-red-600' : 'text-gray-400'}`}
      >
      ...
      <Link 
        to="/contacto"
        className={`flex flex-col items-center gap-1 min-w-[60px] p-1 ${isActive('/contacto') ? 'text-red-600' : 'text-gray-400'}`}
      >
