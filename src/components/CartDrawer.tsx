...
import { useCart, CartItem } from '../context/CartContext'; // Importamos CartItem
...
export const CartDrawer = () => {
  const { 
    isOpen, closeCart, cart, updateQuantity, cartTotal
  } = useCart();
  ...
  const generarMensajeWhatsApp = () => {
    // TIPADO EXPLÍCITO AQUÍ: (item: CartItem)
...
¿Me confirman los datos de cuenta para transferir?`;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-end">
      <div className="absolute inset-0 bg-black/40 backdrop-blur-sm transition-opacity" onClick={closeCart}/>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-white">
          {cart.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-[60%] text-gray-400 gap-4">
              <div className="bg-gray-50 p-6 rounded-full"><MessageCircle className="w-12 h-12 opacity-20" /></div>
                    ...
                    <div key={item.id} className="flex gap-3 p-2 border border-gray-100 rounded-xl bg-white shadow-sm hover:border-gray-200 transition-colors">
                      ...
                      <div className="flex-1 flex flex-col justify-between py-0.5">
                        <div className="flex justify-between items-start gap-2">
                          ...
                          <span className="text-xs font-bold text-slate-900 shrink-0">${subtotal.toFixed(2)}</span>
                        </div>
                        <div className="flex items-center gap-3 mt-2 self-start">
                          <button onClick={() => updateQuantity(item.id, -1)} className="w-6 h-6 bg-gray-100 hover:bg-gray-200 rounded flex items-center justify-center transition-colors text-slate-600"><Minus size={12}/></button>
                          <span className="text-xs font-bold w-4 text-center text-slate-900">{cantidad}</span>
                          <button onClick={() => updateQuantity(item.id, 1)} className="w-6 h-6 bg-slate-900 hover:bg-slate-800 text-white rounded flex items-center justify-center transition-colors"><Plus size={12}/></button>
                        </div>
                      </div>
                    </div>
                  );
              ...
              <div className="mt-8 pt-6 border-t border-gray-100">
                ...
                <div className="space-y-3">
                  <div className="relative">
                    ...
                    <input type="text" name="nombre" placeholder="Tu Nombre Completo" value={formData.nombre} onChange={handleInputChange} className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:border-red-500 outline-none" />
                  </div>
                  <div className="relative">
                    <MapPin className="absolute left-3 top-3 w-4 h-4 text-gray-400" />
                    <input type="text" name="ciudad" placeholder="Ciudad y Dirección" value={formData.ciudad} onChange={handleInputChange} className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:border-red-500 outline-none" />
                  </div>
                  <div className="relative">
                    <CreditCard className="absolute left-3 top-3 w-4 h-4 text-gray-400" />
                    <select name="metodoPago" value={formData.metodoPago} onChange={handleInputChange} className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:border-red-500 outline-none appearance-none">
        ...
        <div className="p-4 border-t bg-white pb-safe shadow-[0_-4px_20px_rgba(0,0,0,0.05)] z-10">
          {cart.length > 0 && (
            <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 mb-4 flex gap-3">
              ...
              <p className="text-[10px] text-blue-800 leading-tight">Tus datos se enviarán cifrados por WhatsApp.</p>
            </div>
          )}
          <div className="flex justify-between items-center mb-4">
            <span className="text-gray-500 font-medium text-sm">Total Estimado</span>
            <span className="text-2xl font-extrabold text-slate-900">${(Number(cartTotal) || 0).toFixed(2)}</span>
          </div>
